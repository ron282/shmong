...
MessageHandler::MessageHandler(Settings * settings, QObject *parent) : QObject(parent),
    /*persistence_(persistence), lurchAdapter_(lurchAdapter),*/ settings_(settings),
...
void MessageHandler::sendMessage(QString const &toJid, QString const &message, QString const &type, bool isGroup)
{
    ...
    else
    {
        ...
        if (shouldSendMsgStanze == true)
        {
            ...
            client_->sendPacket(QXmppMessage("", toJid, message));
        }
